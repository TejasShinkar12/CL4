...
# Create binary classification: High-value (above median) and Low-value (below median)
df['Value_Category'] = (df['Price'] > df['Price'].median()).astype(int)  # 1 for High-value, 0 for Low-value

# Define features (X) and target (y)
X = df[['Avg. Area Income', 'Avg. Area House Age', 'Avg. Area Number of Rooms', 
        'Avg. Area Number of Bedrooms', 'Area Population']].values
y = df['Value_Category'].values
...
model = models.Sequential([
    layers.Conv1D(64, kernel_size=3, activation='relu', input_shape=(X_scaled.shape[1], 1)),
...
# Evaluate the model
test_loss, test_accuracy = model.evaluate(X_test, y  test_loss, test_accuracy)
